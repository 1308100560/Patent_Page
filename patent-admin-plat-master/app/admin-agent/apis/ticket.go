...
// @Accept  application/json
// @Product application/json
// @Router /api/v1/admin-agent/tickets/ids [get]
// @Security Bearer
func (e Ticket) GetTicketByIds(c *gin.Context) {
	s := service.Ticket{}
	err := e.MakeContext(c).
		MakeOrm().
		MakeService(&s.Service).
	...
	ticketList, err := s.GetTicketListByIds(ids)
	if err != nil {
		e.Logger.Error(err)
		e.Error(500, err, err.Error())
		return
	...
	case dtos.TicketTypeReport:
		...
		var _c int64
		rt := c.Query("reportType")
		ticketList, err = rs.GetReportTicketListByTickets(rt, ticketList, &_c)
		if err != nil {
			e.Logger.Error(err)
			e.Error(500, err, err.Error())
			return
...
// @Accept  application/json
// @Product application/json
// @Router /api/v1/admin-agent/tickets/{id} [get]
// @Security Bearer
func (e Ticket) GetTicket(c *gin.Context) {
	s := service.Ticket{}
	err := e.MakeContext(c).
		MakeOrm().
		MakeService(&s.Service).
	...
	if ticket, err = s.GetByID(id); err != nil {
		e.Logger.Error(err)
		e.Error(500, err, err.Error())
		return
	}
	...
	case dtos.TicketTypeReport:
		...
		ticket.RelObj, err = rs.GetReportByTicket(ticket)
		if err != nil {
			e.Logger.Error(err)
			e.Error(500, err, err.Error())
			return
...
func (e Ticket) CreateTicket(c *gin.Context) {
	...
	case dtos.TicketTypeReport:
		...
		req.RelObj.ReportName = req.RelObj.ReportName + "_" + utils.RandStringRunes(5)
		...
		defer func() {
			if ticket != nil {
				...
				// trace
				apis.InternalTrace(c).ReportTracing(ticketDBReq.Name)
