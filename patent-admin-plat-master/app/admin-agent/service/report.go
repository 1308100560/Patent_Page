...
func (e *Report) GetReportByTicket(ticket *model.Ticket) (*model.Report, error) {
	var err error
	tl := model.ReportRelation{}
	err = e.Orm.Model(&tl).
...
func (e *Report) GetReportByIDs(ids []int) ([]model.Report, error) {
	var r model.Report
	reports := make([]model.Report, 0)
	err := e.Orm.Model(&r).
		Find(&reports, ids).Limit(-1).Offset(-1).
		Error
	if err != nil {
		e.Log.Errorf("db error:%s", err)
		return nil, err
