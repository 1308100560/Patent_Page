...
func (e *Report) GetReportListByPatentIDs(pids []int, uid int) ([]model.Report, error) {
	if len(pids) == 0 {
		return []model.Report{}, nil
	}
	var err error
	pl := model.ReportRelation{}
	var pls []model.ReportRelation
	err = e.Orm.Model(&pl).
		Where("user_id = ? AND patent_id IN ?", uid, pids).
		Find(&pls).
		Error
	if err != nil {
		e.Log.Errorf("db error:%s", err)
	...
	reportIDs := make([]int, 0)
	for _, rela := range pls {
		reportIDs = append(reportIDs, rela.ReportId)
	}

	var r model.Report
	var rs []model.Report
	err = e.Orm.Model(&r).
		Find(&rs, reportIDs).Limit(-1).Offset(-1).
		Error
	if err != nil {
		e.Log.Errorf("db error:%s", err)
		return nil, err
...
func (e *Report) GetReportListByTicketIDs(tids []int, uid int) ([]model.Report, error) {
	if len(tids) == 0 {
		return []model.Report{}, nil
	}
	var err error
	...
	var pls []model.ReportRelation
	err = e.Orm.Model(&pl).
		Where("user_id = ? AND ticket_id IN ?", uid, tids).
		Find(&pls).
		Error
	if err != nil {
		e.Log.Errorf("db error:%s", err)
...
func (e *Report) GetReportTicketIDsByPatentIDs(pids []int, uid int) (reportTicketIDs map[int][]int, err error) {
	pl := model.ReportRelation{}
	var pls []model.ReportRelation
	err = e.Orm.Model(&pl).
		Where("user_id = ? AND patent_id IN ?", uid, pids).
	...
	reportTicketIDs = make(map[int][]int)
	for _, rela := range pls {
		if _, ok := reportTicketIDs[rela.PatentID]; ok {
			reportTicketIDs[rela.PatentID] = append(reportTicketIDs[rela.PatentID], rela.TicketId)
		} else {
			reportTicketIDs[rela.PatentID] = []int{rela.TicketId}
		}
	}
