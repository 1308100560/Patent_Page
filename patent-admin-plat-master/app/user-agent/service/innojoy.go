...
const (
	...
	searchListFields     = "TI,AN,AD,PNM,PD,PA,PINN,CL,CD,AR,CLS,INN,ABST,IDX,ICG,IPC"
	searchFullTextFields = "TI,AN,AD,PNM,PD,PA,PINN,CL,CD,AR,CLS,INN,ABST,ABSTEN,CLM,DESCR,ICG,IPC"
...
func refinePatentDetails(pds []*dto.PatentDetail) {
	for _, pd := range pds {
		...
		pd.Inn = strings.Join(strings.Split(pd.Inn, ";")[:len(strings.Split(pd.Inn, ";"))/2], ";")
	}
}
