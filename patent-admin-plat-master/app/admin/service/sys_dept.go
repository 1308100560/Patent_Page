...
// GetDeptPages 获取Dept对象列表
func (e *SysDept) GetDeptPages(req dto.DeptPagesReq, list *[]models.SysDept, count *int64) error {
	var err error
	var data []models.SysDept
	err = e.Orm.Model(&data).
		Scopes(cDto.Paginate(req.GetPageSize(), req.GetPageIndex())).
		Where("dept_name LIKE ?", fmt.Sprintf("%%%s%%", req.Query)).
