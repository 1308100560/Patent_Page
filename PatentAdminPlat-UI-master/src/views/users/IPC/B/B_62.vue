<template>
  <div class="container">
    <div class="my-card">
      <h2 style="text-align: center">焙烤;制作或处理面团的设备;焙烤用面团</h2>
      <el-form ref="infringementForm" :model="form" :rules="formRules">
        <el-form-item label="模型" prop="patentId">
          <el-select v-model="form.patentId" style="width: 100%" value="">
            <el-option v-for="pa in patents" :key="pa.patentId" :label="pa.patentProperties.TI" :value="pa.patentId" />
          </el-select>
        </el-form-item>
        <el-form-item label="prompt" prop="purpose">
          <el-input v-model="form.purpose" rows="6" type="textarea" />
        </el-form-item>
        <el-form-item label="输入" prop="applicant">
          <el-input v-model="form.applicant" rows="6" type="textarea" />
        </el-form-item>
        <el-form-item label="下载文档" prop="contact">
          {{ form.contact }}
        </el-form-item>
        <div style="text-align: center">
          <el-button type="primary" @click="onSubmit">开始生成</el-button>
        </div>
      </el-form>
    </div>
  </div>
</template>
<script>
import { getClaimedPatents } from '@/api/patent'
import { getContactInfo, reportTicketSubmit } from '@/api/report'

export default {
  data() {
    return {
      patents: [],
      patent: '',
      reportList: [],
      form: {
        // 专利标题
        patentId: '',
        contact: '12345678',
        // 评估目的
        purpose: '',
        // 申请人情况
        applicant: ''
      },
      formRules: {
        patentId: [
          { required: true, message: '请选择专利', trigger: 'blur' }
        ],
        purpose: [
          { required: true, message: '请输入评估目的', trigger: 'blur' }
        ],
        applicant: [
          { required: true, message: '请输入申请人', trigger: 'blur' }
        ]
      }
    }
  },
  mounted() {
    getContactInfo().then(res => {
      this.form.contact = res.data.data
    })
    const patentId = this.$route.query.pId
    if (patentId) {
      this.form.patentId = parseInt(patentId)
    }
    getClaimedPatents({ pageSize: 999999 }).then(res => {
      res.data.data.list.map(item => {
        item.patentProperties = JSON.parse(item.patentProperties)
        return item
      })
      console.log(res.data.data.list[0])
      this.patents = res.data.data.list
    })
  },
  methods: {
    onSubmit() {
      this.$refs.infringementForm.validate(valid => {
        if (valid) {
          const patent = this.patents.find(item => item.patentId === this.form.patentId)
          const reportName = patent.patentProperties.TI
          this.loading = true
          const properties = {
            ...this.form, keys: [
              { key: 'title', value: '专利标题' },
              { key: 'contact', value: '联系客服' },
              { key: 'purpose', value: '评估目的' },
              { key: 'applicant', value: '申请人信息' }
            ], type: '侵权报告'
          }
          const relObj = {
            patentId: this.form.patentId,
            reportName: `${reportName}`,
            reportType: '侵权报告'
          }
          const ticketFrom = {
            name: `${reportName}`,
            properties,
            relObj
          }
          reportTicketSubmit(ticketFrom).then(res => {
            if (res.data.code === 500) {
              this.$message({
                message: '当前专利已经申请报告',
                type: 'error'
              })
            } else {
              this.$message({
                message: '提交成功',
                type: 'success'
              })
              this.$router.push({ path: '/ticket/index' })
            }
            this.loading = false
          })
        }
      })
    }
  }
}
</script>
