<template>
  <div class="container">
    <div class="my-card" style="width: 100%">
      <create-package ref="createPack" :get-list="getList" />
      <div style="display: flex;flex-direction: row;align-items: center;justify-content: space-between">
        <div style="display: flex;flex-direction: row;align-items: flex-end">
          <el-input
            v-model="listQuery.query"
            class="filter-item"
            placeholder="专利组合名称名称"
            size="small"
      ...
      >
        ...
        <el-table-column label="专利组合用途" width="200">
          <template slot-scope="{row}">
        ...
        <el-table-column class-name="small-padding fixed-width" fixed="right" label="操作" min-width="120">
          <template slot-scope="{row}">
            <div class="actions" style="display: flex;flex-direction: row;justify-content: space-between">
              ...
              <el-button
                icon="el-icon-plus"
                @click="$router.push(`/patent/claim`)"
              />
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      XLSX.writeFile(workbook, '专利组合.xlsx')
    },
    updatePackageSubmit() {
    ...
    handleDeletePackage(packageId) {
      this.$confirm('此操作将永久删除该专利组合, 是否继续?', '提示', {
